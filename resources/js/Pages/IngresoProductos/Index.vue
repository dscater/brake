...
const { props } = usePage();
const { getIngresoProductosApi, setIngresoProducto, limpiarIngresoProducto, deleteIngresoProducto } =
    useIngresoProductos();
const responseIngresoProductos = ref([]);
const listIngresoProductos = ref([]);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
    ...
    setTimeOutLoadData = setTimeout(async () => {
        responseIngresoProductos.value = await getIngresoProductosApi(options.value);
        listIngresoProductos.value = responseIngresoProductos.value.data;
        totalItems.value = parseInt(responseIngresoProductos.value.total);
        loading.value = false;
    }, 300);
};
const recargaIngresoProductos = async () => {
    loading.value = true;
    listIngresoProductos.value = [];
    options.value.search = search.value;
    responseIngresoProductos.value = await getIngresoProductosApi(options.value);
    listIngresoProductos.value = responseIngresoProductos.value.data;
    totalItems.value = parseInt(responseIngresoProductos.value.total);
...
const editarIngresoProducto = (item) => {
...
const eliminarIngresoProducto = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let respuesta = await deleteIngresoProducto(item.id);
            if (respuesta && respuesta.sw) {
                recargaIngresoProductos();
            }
        }
    });
};
    ...
    <v-container>
        ...
        <v-row class="mt-0">
            <v-col
                ...
                v-if="props.auth.user.permisos.includes('ingreso_productos.create')"
            >
                <v-btn
                    color="primary"
                ...
                <v-card flat>
                    <v-card-title>
                        <v-row class="bg-primary d-flex align-center pa-3">
                            <v-col cols="12" sm="6" md="4"> Ingreso de Productos </v-col>
                                ...
                                <tr v-if="!mobile">
                                    ...
                                    <td class="text-right">
                                        <v-btn
                                            ...
                                            size="small"
                                            class="pa-1 ma-1"
                                            @click="editarIngresoProducto(item)"
                                            icon="mdi-pencil"
                                        ></v-btn>
                                        <v-btn
                                            v-if="
                                            ...
                                            size="small"
                                            class="pa-1 ma-1"
                                            @click="eliminarIngresoProducto(item)"
                                            icon="mdi-trash-can"
                                        ></v-btn>
                                    </td>
                                </tr>
        ...
        <Formulario
            ...
            @envio-formulario="recargaIngresoProductos"
