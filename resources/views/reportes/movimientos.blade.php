...
<head>
    <meta charset="UTF-8">
    <title>Movimientos Ingresos y Egresos</title>
    <style type="text/css">
        * {
...
<body>
    ...
    <div class="encabezado">
        ...
        <h4 class="texto">MOVIMIENTOS DE INGRESOS Y EGRESOS ECONÓMICOS</h4>
    ...
    <table border="1">
        ...
        <tbody>
            ...
            @foreach ($registros as $item)
                ...
                @php
                    if ($item->tipo == 'ingreso') {
                        $ti_c += (float) $item->cantidad;
                        $ti_m += (float) $item->monto;
                        $total_final_c += (float) $item->cantidad;
                        $total_final_m += (float) $item->monto;
                    } else {
                        $te_c += (float) $item->cantidad;
                        $te_m += (float) $item->monto;
                        $total_final_c -= (float) $item->cantidad;
                        $total_final_m -= (float) $item->monto;
                    }
                @endphp
